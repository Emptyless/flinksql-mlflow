...
2. Run the `mlflow` profile to start MLFlow and its dependencies (Minio, PostgreSQL)

`docker compose --profile mlflow up -d`

3. Run the `training` profile to train the PyTorch when mlflow becomes available (check http://localhost:5001 in the browser)

`docker compose --profile training up`

4. Run the `fullstack` profile to run the remainder of the stack (zookeeper, kafka, jobmanager, taskmanager, sqlclient)
...
5. Now that the full stack is running, visit in the browser:
...
6. Go to the kafka-ui in the browser and select the image-stream topic. Produce a message which is a base64 encoded image, e.g.:
